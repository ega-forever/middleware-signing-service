...
].map(privKey => ({secretKey: Buffer.from(privKey, 'hex'), balance: web3.utils.toWei(new web3.utils.BN(10).pow(new web3.utils.BN(20)))}));
...
const web3ProviderUri = `${/^win/.test(process.platform) ? '\\\\.\\pipe\\' : ''}/tmp/development/geth.ipc}`;
